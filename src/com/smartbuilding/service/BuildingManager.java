...
public class BuildingManager {
    ...
    public void saveData() throws FileOperationException {
        ...
        // Persist building state and alerts to disk
    ...
    private void handleAddEquipment() throws Exception {
        ...
        // Display available rooms to help the user pick an ID or name
        System.out.println("Available Rooms:");
        int idx = 1;
        for (Floor f : building.getFloors()) {
            for (Room r : f.getRooms()) {
                System.out.println("  " + idx + ". " + r.getName() + " (ID: " + r.getComponentId() + ")");
                idx++;
            }
        }
    ...
    private void handleUpdateOccupancy() throws Exception {
        ...
        // Display available rooms with current occupancy to help the user pick
        System.out.println("Available Rooms:");
        int idx = 1;
        for (Floor f : building.getFloors()) {
            for (Room r : f.getRooms()) {
                System.out.println("  " + idx + ". " + r.getName() + " (ID: " + r.getComponentId() + ", Occupancy: " + r.getCurrentOccupancy() + "/" + r.getCapacity() + ")");
                idx++;
            }
        }
    ...
    private void handleGenerateReports() throws InvalidAccessException {
        ...
        switch (choice) {
            ...
            case 5:
                ...
                break;
            case 6:
                System.out.print("Enter report types (comma separated). Use numbers (1-5) or names: ");
                ...
                for (String t : raw) {
                    ...
                    switch (tok.toUpperCase()) {
                        case "1": case "ENERGY": case "E": selected.add("ENERGY"); break;
                        case "2": case "OCCUPANCY": case "O": selected.add("OCCUPANCY"); break;
                        case "3": case "EQUIPMENT": case "EQ": selected.add("EQUIPMENT"); break;
                        case "4": case "SECURITY": case "S": selected.add("SECURITY"); break;
                        case "5": case "LIGHTING": case "L": selected.add("LIGHTING"); break;
                        default:
                            System.out.println("Unknown report token ignored: " + tok);
                    }
                }
                if (selected.isEmpty()) {
                    System.out.println("No valid report types selected.");
                } else {
                    generateMultipleReports(selected.toArray(new String[0]));
                }
                break;
            default:
                System.out.println("Invalid choice!");
    ...
    private Room findRoomById(String roomId) {
        ...
        // 1) Try exact component ID match
        ...
        // 3) If user provided a numeric index (1-based) return that room from flattened list
        try {
            ...
            if (idx > 0) {
                ...
                if (idx <= allRooms.size()) {
                    return allRooms.get(idx - 1);
                }
            }
